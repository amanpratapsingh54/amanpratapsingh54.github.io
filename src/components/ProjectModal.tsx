      ...
      {project ? (
        ...
        >
          <motion.article
            ...
            className="max-h-[88vh] w-full max-w-4xl overflow-y-auto rounded-[8px] border border-slate-900/10 bg-white p-6 shadow-card dark:border-white/10 dark:bg-[#0d1117] sm:p-8"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-6">
              <div className="flex items-start gap-4">
                <div className="grid h-14 w-14 shrink-0 place-items-center rounded-[6px] border border-slate-900/10 bg-slate-50 text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-200">
                  <Icon size={28} />
                </div>
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{project.category}</p>
              ...
              <button
                type="button"
                onClick={onClose}
                className="grid h-10 w-10 shrink-0 place-items-center rounded-[6px] border border-slate-900/10 text-slate-500 transition hover:border-slate-400 hover:text-slate-900 dark:border-white/10 dark:text-slate-300 dark:hover:text-white"
              ...
              <aside className="rounded-[8px] border border-slate-900/10 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/[0.04]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="rounded-[6px] bg-white px-3 py-1 text-xs font-medium text-slate-600 dark:bg-white/[0.06] dark:text-slate-200">
                      {tag}
                    </span>
                  ))}
                </div>
...
function ModalLink({ href, icon, label }: { href: string; icon: ReactNode; label: string }) {
  ...
  return (
    <a
      ...
      className="inline-flex items-center justify-center gap-2 rounded-[6px] bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200"
    >
      {icon}
      {label}
    </a>
